...
public class FileuploadClient {
    ...
    //文件需要上传到文件服务器的哪个相对目录下,使用jvm启动参数指定,例如: -Dnetty.client.dirname=9360
    ...
    public static void upload(String host, int port,
        ...
        if (uploadDir.isDirectory()) {
            ...
            ZipUtil.zip(uploadDir, tempFile, true);
            temp = tempFile;
        }
    ...
    private static class FileUploadClientHandler
        ...
        private ChannelFutureListener newListener(final ChannelHandlerContext ctx, final long start, final int currentRead) {
            return new ChannelFutureListener() {
                public void operationComplete(ChannelFuture future) throws Exception {
                    ...
                    if (length == 0) {
                        ...
                        //发送最后的文件传输完成报文
                        ctx.writeAndFlush(transferFinish).addListener(new GenericFutureListener<Future<? super Void>>() {
